...
class Partner:
                ...
                try:
                    ...
                    if (not partner_property):
                        ...
                        if (management.get('is_available')):
                            ...
                            properties_to_add += Management.add(partner_id, management.get('price'),
                                                                management.get('tax_rate', 0),
                                                                management.get('property_tax', 0), False)
                        continue
                    ...
                    if partner_property.get('preco') != management.get('price'):
                        Log.print(
                            "{} - {} com preco de venda alterado. Preco antigo: {} Preco Novo: {}".format(partner_id,
                                                                                                          partner_code,
                                                                                                          management.get(
                                                                                                              'price'),
                                                                                                          partner_property.get(
                                                                                                              'preco')),
                            show_screen=False)
    ...
    def create_query_to_insert_property(property, district_id):
        try:
            ...
            properties_to_add = Property.add(partner_id, partner_code, type_id, district_id, city_id, goal_id, number,
                                             street, size, bedroom_number, room_number, bath_number, parking_number,
                                             new_ad)
    ...
    def search_properties_by_district():
        ...
        try:
            ...
            for district in tqdm(districts):
                ...
                try:
                    properties = Partner.get_properties_by_district('venda', 'bairros', district.get('name', ''),
                                                                    'belo horizonte');
    ...
    def check_property_sold_by_code(code):
        ...
        try:
            ...
            if (not partner_property):
                ...
                management = Management.get_by_partner_id(partner_id)
                properties_to_add += Management.add(partner_id, management.get('price'), management.get('tax_rate', 0),
                                                    management.get('property_tax', 0), False)

                # Check if property value has changed
            management = Management.get_by_partner_id(partner_id)
            if partner_property.get('preco') != management.get('price'):
                Log.print("{} com preco de venda alterado. Preco antigo: {} Preco Novo: {}".format(
                    property.get('partner_code', ''), management.get('price'), partner_property.get('preco')),
                          show_screen=False)
                price = Util.validate_number(partner_property.get('preco'))
                tax_rate = Util.validate_number(partner_property.get('valor_iptu', 0))
                property_tax = Util.validate_number(partner_property.get('valor_condominio', 0))
                properties_to_add += Management.add(partner_id, price, tax_rate, property_tax,
                                                    management.get('is_available'))
            else:
