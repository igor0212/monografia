...
class TestDistrict:
    def test_get_all_when_it_finds_value(self):
        RepositoryDistrict.get_all = MagicMock(return_value=[{'id': 2, 'name': 'Buritis', 'region_id': 3},
                                                             {'id': 3, 'name': 'Padre Eustáquio', 'region_id': 4},
                                                             {'id': 4, 'name': 'Alto Vera Cruz', 'region_id': 2},
                                                             {'id': 5, 'name': 'Jardim dos Comerciários',
                                                              'region_id': 9}])
    ...
    def test_get_all_when_exception_occurs(self):
        ...
        with pytest.raises(Exception, match=r".*District Service - get_all error:*"):
            District.get_all()
    ...
    def test_get_by_id_when_return_is_invalid(self):
        ...
        with pytest.raises(Exception, match=r".*District Service - get_by_id error:*"):
            District.get_by_id(id)
